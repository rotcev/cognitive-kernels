...
export class LensProcessDrawer extends LensElement {
  ...
  private _renderTerminal() {
    return html`<div class="placeholder">Terminal output for ${this.process?.name ?? "unknown"}</div>`;
  }

  private _renderBlackboard() {
    return html`<div class="placeholder">Blackboard entries for ${this.process?.name ?? "unknown"}</div>`;
  }
