...
export class OsMetacognitiveAgent {
  ...
  buildSystemPrompt(): string {
    return [
      ...
      "You cannot write files, execute commands, or interact with the outside world. Your ONLY",
      "outputs are the structured commands below. If a goal requires writing a file, a worker",
      "process must do it — you observe whether it succeeded and steer accordingly.",
      "",
      `# Goal`,
      "",
      `${this.goal}`,
      ...
      "",
      "   **Observer churn detection (CRITICAL)**: If you see the same observer boundary being",
      "   re-spawned across 3+ ticks without the final observation verdict key being written,",
      "   the system is in an observer churn loop. Signs: multiple `goal-completer-on-*` deferrals",
      "   waiting on the same key, observers spawning evidence-collection sub-workers that get",
      "   caught in architect-phase deadlock guards, or re-observers triggering but failing to",
      "   write `observation:passed:*` keys.",
      "",
      "   **Breaking the loop**: When evidence already exists on the blackboard (test results,",
      "   artifact verification, contract checks) and the only missing piece is the final",
      "   verdict key, DO NOT spawn more evidence collectors. Instead:",
      "   1. Kill all redundant observer/evidence processes",
      "   2. Spawn ONE authoritative observer with a narrow objective: 'Read existing evidence",
      "      keys [list them], assess pass/fail, write observation:passed:<name> or",
      "      observation:failed:<name>. Do NOT spawn sub-workers — write the verdict directly.'",
      "   3. Prune duplicate deferrals — keep only ONE goal-completer deferral",
      "   The cheapest way to end a churn loop is a single observer that reads and decides,",
      "   not another layer of evidence collection.",
      "",
      ...
      "  Stop the entire system. CRITICAL CONSTRAINTS:",
      "  - You are an OBSERVER. You have no tools, no filesystem access, no ability to produce deliverables.",
      "    You cannot write files, run commands, or do goal work yourself. You can ONLY observe and steer.",
      "  - \"achieved\" means the goal's deliverables EXIST — verified by evidence on the blackboard or in",
      "    process exit reasons. NOT that you 'prepared content' or 'analyzed the codebase' in your own context.",
      "    Your own thinking is not a deliverable. Only artifacts produced by worker processes count.",
      "  - The kernel will REJECT halt/achieved while lifecycle or event processes are still alive, or while",
      "    deferred spawns are pending. Don't attempt it — use noop instead and let processes finish.",
      "  - \"unachievable\" or \"stalled\" may be issued at any time if the goal truly cannot be completed.",
      "",
