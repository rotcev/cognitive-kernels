...
export class OsMetacognitiveAgent {
  ...
  buildSystemPrompt(): string {
    return [
      ...
      "",
      "topology: Declare the work graph as a JSON object using these node types:",
      "",
      "  Task node:  { type: \"task\", name: \"unique-name\", objective: \"what this worker should do\" }",
      "  Seq node:   { type: \"seq\", children: [node, node, ...] }   — sequential: each child starts when the previous completes",
      "  Par node:   { type: \"par\", children: [node, node, ...] }   — parallel: all children run concurrently",
      "  Gate node:  { type: \"gate\", condition: {...}, child: node } — run child when condition is met",
      "",
      "  Example — two tasks in sequence:",
      '  { "type": "seq", "children": [',
      '    { "type": "task", "name": "list-files", "objective": "List all files in the working directory and write results to blackboard key files:listing" },',
      '    { "type": "task", "name": "summarize", "objective": "Read blackboard key files:listing and write a 2-3 sentence summary to blackboard key result:summary" }',
      "  ]}",
      "",
      "  Example — single task:",
      '  { "type": "task", "name": "do-work", "objective": "Accomplish the goal and write results to the blackboard" }',
      "",
      "  Conditions for gate:",
      '    { type: "blackboard_key_exists", key: "..." }',
      '    { type: "blackboard_key_match", key: "...", value: ... }',
      '    { type: "blackboard_value_contains", key: "...", substring: "..." }',
      ...
      "- Prefer par() for independent research/exploration — e.g. reading different files,",
      "  exploring different aspects of a question, or investigating different subsystems.",
      "- Use seq() when later work depends on earlier output (e.g. gather → synthesize).",
      "- A common pattern: par(research-a, research-b, research-c) → seq(synthesize).",
      "  Each researcher writes findings to the blackboard; the synthesizer reads and combines.",
      ...
      "- Parallelize only when independent. Parallel processes sharing write targets will conflict.",
