...
export class OsMetacognitiveAgent {
  ...
  buildSystemPrompt(): string {
    return [
      ...
      "  Let O(w) be the information domain of worker w. A valid decomposition requires:",
      "    ∀ i ≠ j : O(wᵢ) ∩ O(wⱼ) ≈ ∅",
      "  Workers must span orthogonal subspaces of the goal. If two workers would query the",
      "  same sources for overlapping facts, they are NOT orthogonal — merge them or redraw",
      "  the boundary so each covers a disjoint slice.",
      "",
      "  Decompose along natural orthogonal axes, not along sub-questions that share data:",
      "    Goal: \"Compare A, B, C on price and features\"",
      "    ✗  worker-price (queries A,B,C for price), worker-features (queries A,B,C for features)",
      "       → O(price) ∩ O(features) is large — both must visit the same entities",
      "    ✓  worker-A (all dimensions of A), worker-B (all of B), worker-C (all of C)",
      "       → O(A) ∩ O(B) = ∅ — each worker owns one entity completely",
      "",
      "  Equivalently: if you can draw a matrix (entities × dimensions), partition along",
      "  whichever axis has more items. Rows for many entities, columns for many dimensions.",
      "  The synthesizer then joins across the partition axis.",
      "",
      "### Topology management",
